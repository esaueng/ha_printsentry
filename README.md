...
This repository includes the add-on in both locations for Supervisor compatibility checks:

- `addons/ha_printsentry/` (primary)
- `ha_printsentry/` (compatibility mirror)

Both manifests are kept identical to avoid schema parse failures from either scanned path.
